...
For Nautil components, they will run a `digest` task to generate derived properties such as `attrs` `style` `className` ans so on. After this task and before `render`, a `onDigested` lifecircle function will be called. And, subscribe to event streams should must behind `digest`, so you can only subscribe in `onDigested`.
