...
export default class SampleModel extends Model {
  ...
  /**
   * 接收到detector发来的修改id属性的请求时，可以进行二次修复，本质上也算是钩子函数
   * 它在全局钩子函数（除consume外）被调用之后执行
   * 任何对id发出更改请求的，都会走到这个钩子，包括props的变化，子作用域的反写，detector的通知
  ...
  /**
   * isShow不是model上的属性，因此，它不代表一个属性的修改，它的作用是在流循环中，接收来自detector的一个isShow请求
   * 它仅响应detector的通知
   ...
   * this.fork$()会调起一个新的stream，它的结果会被认为是对model的修改，因此，它就像detector的一个操作一样，会重新触发model上的方法
   * @param {*} stream
   */
  isShow(stream) {
