...
export class Component extends ReactComponent {
  ...
  _polluteRenderTree(tree) {
    ...
    if (process.env.RUNTIME_ENV === 'ssr') {
      ...
      const { __pollutedComponents = [] } = this.props
      pollutedComponents = [].concat(__pollutedComponents).concat(pollutedComponents)

      // current component is pulloted component
      // pass __pollutedComponents to children tree
      if (this._pollutedComponents) {
        const map = (node) => {
          ...
          const { children, __pollutedComponents = [], ...rest } = props
          const subtree = modify(children)
          const polluted = [].concat(__pollutedComponents).concat(this._pollutedComponents)
          return cloneElement(node, { ...rest, __pollutedComponents: polluted }, subtree)
        }
        const modify = (tree) => {
          if (isArray(tree)) {
