...
export class Component extends React.Component {
  ...
  _digest(props) {
    ...
    if (comsumeProviders && this.consumeRender) {
      let consumer = null
      const providers = {}
      each(comsumeProviders, (value, key) => {
        ...
        providers[key] = value
        const prevConsumer = consumer
        const childFn = () => {
          return prevConsumer || this.children(providers)
        }
        consumer = React.createElement(Consumer, { name: key }, childFn)
      })
