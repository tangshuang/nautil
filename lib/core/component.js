...
export class Component extends ReactComponent {
  constructor(props) {
    ...
    this._pollutedProps = Constructor._pollutedProps
    this._scrawledStyles = Constructor._scrawledStyles
  ...
  _digest(props) {
    ...
    const pollution = this._pollutedProps || {}
    const graffiti = this._scrawledStyles || []
    const { children, stylesheet, style, className, ..._attrs } = props

    /**
     * Format stylesheet by using stylesheet, className, style props
     */
    const stylequeue = [].concat(graffiti).concat(defaultStylesheet).concat(stylesheet).concat(className).concat(style)
    const classNames = []
    const styles = {}
