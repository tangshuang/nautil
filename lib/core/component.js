...
export class Component extends ReactComponent {
  constructor(props) {
    ...
    this.forceUpdate = (callback) => {
      ...
      return forceUpdate(callback)
    }
    this.update = (callback) => this.setState({}, callback)

    // render
    const _render = this.render.bind(this)
    this.render = () => {
      const tree = _render()
      const polluted = this.shouldPollute() ? this._polluteRenderTree(tree) : tree
      return polluted
    }
  ...
  _polluteRenderTree(tree) {
    ...
    const modify = (tree) => {
      const modifed = mapChildren(tree, (node) => {
