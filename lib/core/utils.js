import { isObject, isArray, isString, isFunction, isInstanceOf, createProxy, assign } from 'ts-fns'
...
export function filterChildren(children, iterator) {
  ...
  children = Children.toArray(children).filter(iterator)
  return children
}
 ...
 * @param {*} react
 */
export function createTwoWayBinding(data, react) {
  const createReflect = (data, keyPath, target, key) => {
    return  v => {
      // use passed react
      if (isFunction(react)) {
        react([data, keyPath, v], [target, key, v])
      }
