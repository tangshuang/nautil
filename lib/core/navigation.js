import { inObject, isArray, isString, isObject, isFunction } from './utils.js'
...
export class Navigation {
  ...
  dispatch(event, state) {
    ...
    this._listeners.forEach((item) => {
      ...
      // i.e. on('!', callback)
      else if (match === '!' && event === '$onNotFound') {
      ...
      else if (event === '$onEnter' && isString(match) && state) {
        const { name, url } = state
        if (match === name) {
          callback.call(this, state)
        }
        // i.e. on('parent', callback) and dispatch parent.child
      ...
      else if (event === '$onEnter' && match instanceof RegExp && state) {
        const { url } = state
        if (match.test(url)) {
          callback.call(this, state)
        }
      }
      // i.e. on(func, callback)
      else if (event === '$onEnter' && isFunction(match) && state) {
        if (match(state)) {
          callback.call(this, state)
        }
      }
  ...
  test(match, exact = false) {
    ...
    if (match instanceof RegExp && match.test(url) && !exact) {
      return true
    }
    if (isFunction(match)) {
      return match(state)
