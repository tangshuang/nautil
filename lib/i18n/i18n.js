...
if (process.env.RUNTIME_ENV === 'web' || process.env.RUNTIME_ENV === 'web-component'|| process.env.RUNTIME_ENV === 'miniapp' || process.env.RUNTIME_ENV === 'ssr-client') {
  LanguageDetector = require('i18next-browser-languagedetector')
}
else if (process.env.RUNTIME_ENV === 'native') {
  LanguageDetector = require('i18next-react-native-language-detector')
}
...
export class I18n {
  ...
  init(options = {}) {
    ...
    // disable in SSR
    if (detector && process.env.RUNTIME_ENV !== 'ssr') {
