...
export function define(name, Component, cssText) {
  ...
  class ReactCustomElement extends HTMLElement {
    ...
    mount() {
      const props = {
        ...this._getProps(this.attributes, Component.validateProps),
        ...this._getEvents(Component.validateProps),
      }
    ...
    _getEvents(validateProps) {
      return Object.keys(validateProps).filter(key => /on([A-Z].*)/.exec(key))
        .reduce((events, ev) => ({
          ...events,
    ...
    _getProps(attributes, validateProps = {}) {
      return [ ...attributes ].filter(attr => attr.name !== 'style')
         .map(attr => this._convertProp(attr.name, attr.value, validateProps))
         .reduce((props, prop) => ({ ...props, [prop.name]: prop.value }), {})
   }
   _convertProp(attrName, attrValue, validateProps) {
      const propName = Object.keys(validateProps).find(key => key.toLowerCase() == attrName)
      let value = attrValue
